...
Converter = str  # "buck", "boost", "buckboost"
...
def _modes(spec: ConverterSpec):
    """Return dict of mode -> (A, b, cout) with vout = cout @ x."""
...
def steady_state(spec: ConverterSpec, steps_per_period: int = 200, iters: int = 30):
    ...
    Newton converges in one step; in DCM a few iterations are needed."""
    modes = _modes(spec)
...
def simulate(spec: ConverterSpec, n_periods: int = 20, steps_per_period: int = 200,
    ...
    Metrics are computed on the last 5 switching periods.
    """
    ...
    ic = iL[s] * (sw[s] != 1 if top != "buck" else 1) - vout[s] / spec.R
    res.losses = {
